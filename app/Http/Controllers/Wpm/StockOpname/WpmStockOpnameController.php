...
use App\Models\Wpm\StockOpname\WpmSoTempNoteModel;
use App\Models\Wpm\StockOpname\WpmSoStatusModel;
use App\Models\Wpm\StockOpname\WpmSoApprovalModel;
...
class WpmStockOpnameController extends Controller
{
    public function startOpname(Request $request)
    {
        ...
        if ($jenisSo === 'monthly') {
            ...
            $existingMonthlyThisMonth = WpmSoStatusModel::where('jenis_so', 'monthly')
        ...
        $sohCount = WpmSohModel::whereDate('created_at', $today)
        ...
        $existing = WpmSoStatusModel::whereDate('tgl_opname', $today)
            ->where('jenis_so', $jenisSo)
            ->first();
        ...
        $status = WpmSoStatusModel::create([
            'user_id' => $user->id ?? 1,
            'tgl_opname' => $today,
            'status' => 'started',
            'jenis_so' => $jenisSo,
    ...
    {
        $today = now()->toDateString();
        $status = WpmSoStatusModel::whereDate('tgl_opname', $today)
            ->where('jenis_so', $jenisSo)
            ->first();
    ...
    public function getStatusOpname(Request $request)
    {
        $today = now()->toDateString();
        $jenisSo = $request->input('jenis_so', 'cycle_count');
        $status = WpmSoStatusModel::with('user')
            ->whereDate('tgl_opname', $today)
            ->where('jenis_so', $jenisSo)
            ->first();
    ...
    public function getData(Request $request)
    {
        ...
        $query = WpmSohModel::query()
            ...
            ->whereDate('wpm_soh.created_at', $today)
            ->where('wpm_soh.jenis_so', $jenisSo);

        if ($search) {
            $query->where(function ($q) use ($search) {
                $q->where('wpm_master_barang.mid', 'like', '%' . $search . '%')
                    ->orWhere('wpm_master_barang.nama_barang', 'like', '%' . $search . '%');
            });
        }
        ...
        $tempData = WpmSoTempModel::whereDate('tgl_opname', $today)
            ->whereHas('soh', function ($q) use ($jenisSo) {
                $q->where('jenis_so', $jenisSo);
            })
            ->get()->groupBy('barang_id');

        $tempNotes = WpmSoTempNoteModel::whereDate('tgl_opname', $today)
            ->whereHas('soh', function ($q) use ($jenisSo) {
                $q->where('jenis_so', $jenisSo);
            })
            ->get()->keyBy('barang_id');
    ...
    public function saveTemp(Request $request)
    {
        ...
        $soh = WpmSohModel::with('barang')->findOrFail($request->soh_id);
        ...
        if ($request->has('keterangan')) {
            ...
            if ($catatan !== '') {
                WpmSoTempNoteModel::updateOrCreate(
                    [
                        'soh_id' => $soh->id,
                        'barang_id' => $soh->barang_id,
                        'tgl_opname' => $today
                ...
                WpmSoTempNoteModel::where('soh_id', $soh->id)
                    ->whereDate('tgl_opname', $today)
                    ->delete();
            }
        }
        ...
        $allTemps = WpmSoTempModel::where('soh_id', $soh->id)
            ->whereDate('tgl_opname', $today)
            ->get();
    ...
    public function getDataTempBatch(Request $request)
    {
        ...
        // Get Qty
        $qtyRecords = WpmSoTempModel::with(['barang', 'soh.barang'])
            ->whereDate('tgl_opname', $today)
        ...
        // Get Note
        $noteRecords = WpmSoTempNoteModel::with(['barang', 'soh.barang'])
            ->whereDate('tgl_opname', $today)
            ->where(function ($q) use ($sohIds, $barangIds) {
                if (!empty($sohIds)) {
                    $q->whereIn('soh_id', $sohIds);
    ...
    public function getDataTempEdit(int $sohId)
    {
        ...
        $dataQty = WpmSoTempModel::with('barang')
            ->where('soh_id', $sohId)
            ->whereDate('tgl_opname', $today)
            ->orderBy('updated_at', 'asc')
            ->get();

        $dataNote = WpmSoTempNoteModel::where('soh_id', $sohId)
            ->whereDate('tgl_opname', $today)
            ->latest('updated_at')
            ->first();
    ...
    public function updateTempBatch(Request $request)
    {
        ...
        $firstTemp = WpmSoTempModel::find($items[0]['id']);
        $jenisSo = 'cycle_count';
            ...
            if ($catatan !== null) {
                foreach ($processedPairs as $pair) {
                    if (trim($catatan) !== '') {
                        ...
                        WpmSoTempNoteModel::where('soh_id', $pair['soh_id'])
    ...
    public function destroyTemp($id, Request $request)
    {
        ...
        if ($type === 'note') {
            $note = WpmSoTempNoteModel::where('soh_id', $id)->first();
            if ($note && $note->soh) {
                $jenisSo = $note->soh->jenis_so;
            }
        } else {
            $temp = WpmSoTempModel::with('soh')->find($id);
            if ($temp && $temp->soh) {
        ...
        try {
            if ($type === 'note') {
                $deleted = WpmSoTempNoteModel::where('soh_id', $id)->delete();
                if (!$deleted) {
                    return response()->json([
                        'status' => 'error',
    ...
    public function saveTempNew(Request $request)
    {
        ...
        $soh = WpmSohModel::updateOrCreate(
            ...
            [
                'user_id' => $user->id ?? 1,
                'qty_soh' => round((float)$request->unrest + (float)$request->qi + (float)$request->blocked, 2),
        ...
        // Save to temp opname
        $temp = WpmSoTempModel::updateOrCreate(
            [
                'soh_id' => $soh->id,
                'barang_id' => $barang->id,
                'tgl_opname' => $today
    ...
    public function resetTempRow(Request $request)
    {
        ...
        $soh = WpmSohModel::findOrFail($request->soh_id);
        ...
        $today = now()->toDateString();
        WpmSoTempModel::where('soh_id', $request->soh_id)->whereDate('tgl_opname', $today)->delete();
        WpmSoTempNoteModel::where('soh_id', $request->soh_id)->whereDate('tgl_opname', $today)->delete();
    ...
    public function processOpname(Request $request)
    {
        ...
        if ($jenisSo === 'monthly' && strlen($tglOpname) === 7) {
            ...
            $statusRecord = WpmSoStatusModel::where('jenis_so', 'monthly')
        ...
        $tempData = WpmSoTempModel::with('barang')
        ...
        $sohData = WpmSohModel::whereDate('created_at', $tglOpname)
            ->where('jenis_so', $jenisSo)
            ->get()
            ->keyBy('barang_id');

        // Pull active temp notes
        $tempNotes = WpmSoTempNoteModel::whereDate('tgl_opname', $tglOpname)
            ->whereHas('soh', function ($q) use ($jenisSo) {
                $q->where('jenis_so', $jenisSo);
            })
            ->get()
        ...
        if ($request->mode === 'final_submit') {
            ...
            DB::beginTransaction();
            try {
                $existingSop = WpmSoModel::whereDate('tgl_opname', $tglOpname)
                    ->where('jenis_so', $jenisSo)
                    ->first();
                if ($existingSop && $existingSop->no_doc) {
                    ...
                    $count = WpmSoModel::whereMonth('tgl_opname', $tanggalCarbon->month)
                ...
                WpmSoDetailModel::where('so_id', $sop->id)->delete();
                WpmSoSummariesModel::where('so_id', $sop->id)->delete();
                WpmSoApprovalModel::where('so_id', $sop->id)->delete();

                WpmSoApprovalModel::create([
                    'so_id' => $sop->id,
                    'approver_id' => $user->id,
                ...
                WpmSoStatusModel::updateOrCreate(
                    [
                        'tgl_opname' => $tglOpname,
                        'jenis_so'   => $jenisSo
                    ],
                ...
                WpmSoTempModel::where('tgl_opname', $tglOpname)
                    ->whereHas('soh', function ($q) use ($jenisSo) {
                        $q->where('jenis_so', $jenisSo);
                    })->delete();

                WpmSoTempNoteModel::where('tgl_opname', $tglOpname)
                    ->whereHas('soh', function ($q) use ($jenisSo) {
                        $q->where('jenis_so', $jenisSo);
                    })->delete();
    ...
    public function getDataReport(Request $request)
    {
        ...
        $query = WpmSoModel::where('jenis_so', $jenisSo);
        ...
        // Get all WpmSoModel records that are not approved yet (draft, pending, rejected)
        $unapprovedSops = WpmSoModel::where('status', '!=', 'approved')
            ->where('jenis_so', $jenisSo)
        ...
        $data = WpmSoSummariesModel::where('so_id', $sop->id)
    ...
    public function getPendingApprovals(Request $request)
    {
        ...
        $approvals = WpmSoApprovalModel::with([
            'approver:id,nama_lengkap,username,jabatan',
            'so:id,tgl_opname,status,user_id,jenis_so',
            'so.user:id,username,nama_lengkap',
        ])
            ->where('approver_id', $user->id)
            ->whereIn('wpm_so_approvals.status', ['pending', 'read'])
            ->join('wpm_so', 'wpm_so.id', '=', 'wpm_so_approvals.so_id')
            ->where('wpm_so.status', '!=', 'approved')
            ->orderByDesc('wpm_so.tgl_opname')
            ->orderByDesc('wpm_so_approvals.created_at')
            ->select('wpm_so_approvals.*')
            ->get();
    ...
    {
        $request->validate([
            'so_id' => 'required|exists:wpm_so,id',
        ...
        $so = WpmSoModel::where('id', $soId)->first();
        ...
        try {
            ...
            $existingApproval = WpmSoApprovalModel::where('so_id', $so->id)
                ->where('approver_id', $user->id)
                ->first();

            $oldNote = $existingApproval->catatan ?? null;

            WpmSoApprovalModel::updateOrCreate([
                'so_id' => $so->id,
                'approver_id' => $user->id,
            ], [
                'status' => 'approved',
            ...
            WpmSoApprovalModel::where('so_id', $so->id)
                ->where('approver_id', '!=', $user->id)
                ->delete();
            ...
            foreach ($approverIds as $approverId) {
                ...
                $approvals[$approverId] = WpmSoApprovalModel::create([
                    'so_id'      => $so->id,
            ...
            $title   = 'Approval SO WPM';
            $message = 'SO Warehouse Packaging Material tanggal ' . $so->tgl_opname . ' menunggu persetujuan Anda.';
            $url     = route('wpm.stock_opname.report') . '?tgl_opname=' . $so->tgl_opname . '&jenis_so=' . $so->jenis_so;
            ...
            foreach ($approverIds as $approverId) {
                ...
                NotificationsModel::create([
                    'user_id'         => $approverId,
                    'notifiable_type' => WpmSoApprovalModel::class,
    ...
    {
        $so = WpmSoModel::findOrFail($id);
        $user = Auth::user();

        $approvals = WpmSoApprovalModel::with('approver:id,nama_lengkap,username,jabatan')
            ->where('so_id', $id)
            ->get();
    ...
    public function updateStatus(Request $request)
    {
        ...
        $approval = WpmSoApprovalModel::where('so_id', $request->so_id)
            ->where('approver_id', $user->id)
            ->first();
        ...
        try {
            ...
            NotificationsModel::where('notifiable_type', WpmSoApprovalModel::class)
                ->where('notifiable_id', $approval->id)
                ->where('user_id', $user->id)
                ->delete();

            $approvals = WpmSoApprovalModel::where('so_id', $request->so_id)->get();
            ...
            $so = WpmSoModel::findOrFail($request->so_id);
            $so->update([
                'status' => $finalStatus
            ]);
            ...
            if (in_array($finalStatus, ['approved', 'rejected']) && $so->user_id) {
                ...
                $titleText = "SO WPM " . ($finalStatus === 'approved' ? 'Approved' : 'Rejected');
                $messageText = "Stock Opname Warehouse Packaging Material tanggal " . $so->tgl_opname . " telah " . $statusText . " oleh " . $user->nama_lengkap . ".";
                ...
                NotificationsModel::where('user_id', $so->user_id)
                    ->where('url', $url)
                    ->where('title', 'like', 'SO WPM%')
                    ->delete();

                NotificationsModel::create([
                    'user_id'         => $so->user_id,
                    'notifiable_type' => WpmSoModel::class,
                    'notifiable_id'   => $so->id,
                    'title'           => $titleText,
                    'message'         => $messageText,
                    'url'             => $url,
    ...
    {
        $summary = WpmSoSummariesModel::with('barang')->findOrFail($id);

        $details = WpmSoDetailModel::where('so_id', $summary->so_id)
    ...
    public function updateReportRow(Request $request, int $id)
    {
        ...
        try {
            ...
            foreach ($items as $it) {
                $detail = WpmSoDetailModel::where('id', $it['id'])
    ...
    public function deleteReportRow(int $id)
    {
        DB::beginTransaction();
        try {
            $summary = WpmSoSummariesModel::findOrFail($id);
            ...
            WpmSoDetailModel::where('so_id', $summary->so_id)
                ->where('barang_id', $summary->barang_id)
                ->delete();
    ...
    public function deleteReportDetail(int $id)
    {
        DB::beginTransaction();
        try {
            $detail = WpmSoDetailModel::findOrFail($id);
            ...
            // Recalculate summary
            $summary = WpmSoSummariesModel::where('so_id', $detail->so_id)
                ->where('barang_id', $detail->barang_id)
                ->first();

            if ($summary) {
                // Get all remaining details for this summary
                $remainingDetails = WpmSoDetailModel::where('so_id', $summary->so_id)
                    ->where('barang_id', $summary->barang_id)
                    ->get();
    ...
    // export
    public function exportPdfSOWPM(Request $request)
    {
        $tglOpname = $request->input('tgl_opname', now()->toDateString());
        $jenisSo = $request->input('jenis_so', 'cycle_count');
        ...
        $summaries = WpmSoSummariesModel::where('so_id', $so->id)
            ->with('barang:id,mid,nama_barang,uom')
            ->get();
        ...
        $approvals = WpmSoApprovalModel::with('approver.signature')
            ->where('so_id', $so->id)
            ->get();
        ...
        $pdf = Pdf::loadView('pdf.so_wpm_report', [
            'so' => $so,
            'summaries' => $summaries,
            'tglOpname' => $tglOpname,
            'approvers' => $approvers,
        ...
        return $pdf->stream('SO_WPM_Report_' . $tglOpname . '.pdf');
    }
}
