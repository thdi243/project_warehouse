...
class WpmStockOnHandController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($jenisSo === 'monthly') {
            ...
            $hasMonthlySo = WpmSoStatusModel::where('jenis_so', 'monthly')
        ...
        try {
            ...
            $sop = WpmSoModel::whereDate('tgl_opname', $today)
                ->where('jenis_so', $jenisSo)
                ->first();
            if ($sop) {
    ...
    public function resetAll(Request $request)
    {
        ...
        if ($jenisSo === 'monthly') {
            $currentYear = now()->year;
            $currentMonth = now()->month;
            $soStatus = WpmSoStatusModel::where('jenis_so', 'monthly')
                ->whereYear('tgl_opname', $currentYear)
                ->whereMonth('tgl_opname', $currentMonth)
        ...
        try {
            $query = WpmSohModel::where('jenis_so', $jenisSo);
            if ($jenisSo === 'monthly') {
            ...
            if ($hasTemp) {
                \App\Models\Wpm\StockOpname\WpmSoTempModel::whereIn('soh_id', $sohIds)->delete();
                \App\Models\Wpm\StockOpname\WpmSoTempNoteModel::whereIn('soh_id', $sohIds)->delete();
            }
