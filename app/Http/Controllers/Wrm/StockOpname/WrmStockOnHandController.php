...
class WrmStockOnHandController extends Controller
{
    ...
    public function resetAll(Request $request)
    {
        ...
        if ($jenisSo === 'monthly') {
            $currentYear = now()->year;
            $currentMonth = now()->month;
            $soStatus = WrmSoStatusModel::where('jenis_so', 'monthly')
                ->whereYear('tgl_opname', $currentYear)
                ->whereMonth('tgl_opname', $currentMonth)
        ...
        try {
            $query = WrmSohModel::where('jenis_so', $jenisSo);
            if ($jenisSo === 'monthly') {
            ...
            if ($hasTemp) {
                \App\Models\Wrm\StockOpname\WrmSoTempModel::whereIn('soh_id', $sohIds)->delete();
                \App\Models\Wrm\StockOpname\WrmSoTempNoteModel::whereIn('soh_id', $sohIds)->delete();
            }
