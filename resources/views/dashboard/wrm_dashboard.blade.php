...
@section('content')
        ...
        <div class="container-fluid">
            ...
            <div class="row mb-4 align-items-center">
                <div class="col-xl-4 col-lg-3 mb-3 mb-lg-0">
                    ...
                    <p class="text-muted mb-0 small"><i class="bx bx-info-circle me-1"></i>Live enterprise inventory tracking
                    </p>
                </div>
            </div>
            ...
            <div class="row mb-4 align-items-end g-2">
                ...
                <div class="col-md-2">
                    <label class="form-label small mb-1">No SPB</label>
                    <select id="filterNoSpb" class="form-select">
                        <option value="">All SPB</option>
                        @foreach ($noSpb as $spb)
                            @if ($spb->no_spb)
                                <option value="{{ $spb->no_spb }}">
                                    {{ $spb->no_spb }}
                                </option>
                            @endif
                        @endforeach
                ...
                <div class="col-md-2">
                    <label class="form-label small mb-1 opacity-0">Action</label>
                    ...
                    <div class="d-flex gap-2 text-nowrap">
                        ...
                        <button id="btnReset" class="btn btn-danger flex-fill">
            ...
            <div class="row mb-4 g-3">
                <div class="col-xl-8">
                    <div class="card dashboard-card h-100 p-3">
                        <h6 class="card-title fw-bold mb-0"><i class="bx bx-line-chart me-2 text-primary"></i>Stock
                            Movement
                            Overview</h6>
                ...
                <div class="col-xl-4">
                    <div class="card dashboard-card h-100 p-3">
                        <h6 class="card-title fw-bold mb-0"><i class="bx bx-pie-chart-alt-2 me-2 text-primary"></i>Stock
                            by
                            Gudang</h6>
            ...
            <div class="row mb-4 g-3">
                ...
                <div class="col-xl-4">
                    <div class="card dashboard-card h-100 p-3">
                        <h6 class="card-title fw-bold mb-0"><i class="bx bx-time-five me-2 text-primary"></i>Aging Stock
                        </h6>
                                    ...
                                    <tbody id="tableRecent">
                                        <tr>
                                            <td colspan="5" class="text-center py-4 text-muted"><i
                                                    class="bx bx-loader bx-spin me-2"></i>Loading...</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div class="card dashboard-card border-0">
                        ...
                        <div class="card-body p-3">
                            ...
                            <div class="d-flex gap-2 mb-4 flex-wrap" id="locationSummary">
                                <div class="px-3 py-1 rounded-pill text-white shadow-sm d-flex align-items-center gap-1"
                                    style="background:#1e293b;font-size:12px;">
                                    <i class="bx bx-border-all"></i> Total: <b id="sumTotal">-</b>
                                </div>
                                <div class="px-3 py-1 rounded-pill text-white shadow-sm d-flex align-items-center gap-1"
                                    style="background:#3b82f6;font-size:12px;">
                                    <i class="bx bx-package"></i> Occupied: <b id="sumOccupied">-</b>
                                </div>
                                <div class="px-3 py-1 rounded-pill shadow-sm d-flex align-items-center gap-1"
                                    style="background:#def7ec;color:#03543f;border:1px solid #84e1bc;font-size:12px;">
                                    <i class="bx bx-check-circle"></i> Available (Reuse): <b id="sumAvailable">-</b>
                                </div>
                                <div class="px-3 py-1 rounded-pill shadow-sm d-flex align-items-center gap-1"
                                    style="background:#fdf2f2;color:#9b1c1c;border:1px solid #f8b4b4;font-size:12px;opacity:0.8;">
                                    <i class="bx bx-time"></i> Pending Outbound: <b id="sumReserved">-</b>
                                </div>
                                <div class="px-3 py-1 rounded-pill shadow-sm d-flex align-items-center gap-1"
                                    style="background:#f1f5f9;color:#475569;border:1px solid #cbd5e1;font-size:12px;display:none;">
    ...
    {{-- Rack Detail Modal --}}
    <div class="modal fade" id="zonaDetailModal" tabindex="-1" aria-labelledby="zonaDetailModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-light border-bottom-0">
                    <h5 class="modal-title fw-bold" id="zonaDetailModalLabel"><i
                            class="bx bx-grid me-2 text-primary"></i> Zona Details</h5>
...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
            ...
            if ($.fn.select2) {
                $('#filterGudang, #filterMid, #filterNoSpb').select2({
                    theme: 'bootstrap-5',
                    width: '100%',
                    allowClear: true,
                    placeholder: 'Select...'
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status) {
                            ...
                            animCounter($('#kpiDraftOutboundToday'), res.data.draft_outbound_today ||
                                0);
            ...
            function fetchTableRecent(params) {
                const $t = $('#tableRecent');
                $t.html(
                    '<tr><td colspan="5" class="text-center py-4 text-muted"><i class="bx bx-loader bx-spin me-2"></i>Loading recent activities...</td></tr>'
                );
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status) {
                            ...
                            if (res.data.length === 0) {
                                $t.append(
                                    '<tr><td colspan="5" class="text-center py-4 text-muted">No recent activities on this filter.</td></tr>'
                                );
                            } else {
                                let html = '';
                                res.data.forEach(item => {
                                    const typeBadge = item.jenis === 'IN' ?
                    ...
                    error: function() {
                        $t.html(
                            '<tr><td colspan="5" class="text-center py-4 text-danger">Failed to load activities.</td></tr>'
                        );
                    }
                });
            }
            ...
            window.openZonaModal = function(gudangKey, zonaName) {
                $('#zonaDetailModalLabel').html(
                    `<i class="bx bx-buildings text-primary me-2"></i> ${gudangKey} <i class="bx bx-chevron-right text-muted mx-1"></i> ZONA <b class=">${zonaName}</b> Details`
                );
                ...
                zInfo.racks.forEach(loc => { // loc is rack
                    ...
                    for (let lvl = maxLevel; lvl >= 1; lvl--) {
                        ...
                        gridHtml +=
                            `<span class="rack-row-label" style="width:20px;text-align:right;">L${lvl}</span>`;
                        for (let col = 1; col <= maxKolom; col++) {
                            ...
                            if (c) {
                                ...
                                if (c.status === 'empty') {
                                    ...
                                    bg = globalUsedMids[c.mid] ? globalUsedMids[c.mid].color :
                                        '#3b82f6';
                                }
                                ...
                                if (c.status === 'occupied') {
                                    tooltip +=
                                        `<br/>SPB: ${c.no_spb}<br/>Pallet: ${c.pallet_id}<br/>Barang: ${c.mid} - ${c.nama_barang}<br/>Qty: ${c.qty}<br/>In: ${c.incoming_date}`;
                                }
                                ...
                                gridHtml +=
                                    `<div style="width:52px;height:52px;border:1px solid transparent;background:transparent;"></div>`;
                            }
                        }
                        gridHtml += `</div>`;
                    }
                    ...
                    gridHtml +=
                        `<div class="d-flex align-items-center gap-1 mt-1"><span style="width:20px;"></span>`;
                    for (let col = 1; col <= maxKolom; col++) {
                        gridHtml +=
                            `<div style="width:52px;text-align:center;font-size:10px;font-weight:600;color:#94a3b8;">C${col}</div>`;
                    }
            ...
            $('#btnReset').on('click', function(e) {
                e.preventDefault();
                $('#filterGudang, #filterMid, #filterNoSpb').val(null).trigger('change');
                const today = new Date();
                const start30 = new Date();
                start30.setDate(today.getDate() - 30);
                $('#filterStartDate').val(start30.toISOString().split('T')[0]);
