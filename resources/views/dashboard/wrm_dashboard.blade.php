                            ...
                            <div class="d-flex align-items-center gap-3 flex-wrap">
                                ...
                                <div class="d-flex align-items-center gap-2" style="font-size:12px;">
                                    <span class="d-inline-block rounded-1"
                                        style="width:16px;height:16px;background:#22c55e;"></span> Occupied
                                    <span class="d-inline-block rounded-1"
                                        style="width:16px;height:16px;background:#f59e0b;"></span> Reserved
                                    <span class="d-inline-block rounded-1"
                                        style="width:16px;height:16px;background:#e2e8f0;border:1px solid #cbd5e1;"></span>
                                    Empty
                                </div>
                            </div>
                        </div>
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        for (const [gudangKey, locs] of Object.entries(byGudang)) {
                            ...
                            <h6 class="mb-0 fw-bold" style="color:#1e293b;">${gudangKey}</h6>
                            ...
                            locs.forEach(loc => {
                                ...
                                for (let lvl = maxLevel; lvl >= 1; lvl--) {
                                    ...
                                    gridHtml +=
                                        `<span style="width:26px;font-size:10px;color:#94a3b8;font-weight:700;text-align:right;flex-shrink:0;">L${lvl}</span>`;
                                        ...
                                        if (cell) {
                                            const statusLabel = cell.status === 'occupied' ?
                                                '🟢 Berisi Stok' : (cell.status === 'reserved' ?
                                                    '🟡 Reserved' : '⬜ Kosong');
                                            gridHtml += `
                                        <div class="rack-cell ${cell.status}" title="">
                                            ${cell.label}
                                            <div class="rack-cell-tooltip">${statusLabel}<br>Bin: ${loc.bin}<br>Kolom ${cell.kolom}, Lvl ${cell.level}</div>
                                        </div>`;
                                        } else {
                                            gridHtml +=
                                ...
                                for (let kol = 1; kol <= maxKolom; kol++) {
                                    gridHtml +=
                                        `<div style="width:52px;text-align:center;font-size:10px;color:#94a3b8;font-weight:600;">K${kol}</div>`;
                                }
                                gridHtml += `</div>`;
                            ...
                            <div class="col-auto">
                                <div class="rack-section">
                                    <div class="rack-section-header">
                                        ...
                                        <span style="font-size:11px;color:#64748b;">${loc.cells.length} posisi</span>
                                    </div>
                                    ${gridHtml}
                                </div>
                            </div>`;
