    ...
    <div class="container-fluid">
        ...
        <div class="row mb-4">
            <div class="col-12 d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-1 fw-bold">Dashboard TKBM BPS</h4>
                    <p class="text-muted mb-0">Statistik Operasional BPS (Terpal, Slipsheet, Pallet)</p>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('tkbm.stock') }}" class="btn btn-soft-primary">
                        <i class="bx bx-plus me-1"></i> Input Data
                    </a>
                    <a href="{{ route('tkbm.data') }}" class="btn btn-soft-info">
                ...
                <div class="card card-animate shadow-sm border-0 rounded-4 overflow-hidden">
                    <div class="card-body" style="background: linear-gradient(135deg, #10b981, #34d399);">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <h6 class="text-white mb-0 opacity-75">Total Produk</h6>
                        </div>
                        <div class="d-flex align-items-end justify-content-between">
                            <div>
                                <h3 class="text-white fw-bold mb-1" id="stat-total-produk">0</h3>
                                <span class="text-white-50 small">(Pallet + Slipsheet + Terpal) Pcs</span>
                            </div>
                ...
                <div class="card card-animate shadow-sm border-0 rounded-4 overflow-hidden">
                    <div class="card-body" style="background: linear-gradient(135deg, #6366f1, #8b5cf6);">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <h6 class="text-white mb-0 opacity-75">Total Qty</h6>
                        </div>
                        <div class="d-flex align-items-end justify-content-between">
                            <div>
                                <h3 class="text-white fw-bold mb-1" id="stat-total-qty">0</h3>
                                <span class="badge bg-glass text-white" id="stat-qty-trend">
        ...
        <div class="row">
            ...
            <div class="col-xl-8">
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-header bg-transparent border-0 pt-4 px-4">
                        <h5 class="card-title mb-0 fw-bold">Tren Harian (Terpal, Slipsheet, Pallet)</h5>
                        <p class="text-muted small mb-0">Distribusi quantity per hari di bulan ini</p>
                    </div>
                    <div class="card-body">
                        <div id="daily-trend-chart" style="min-height: 400px;"></div>
                    </div>
                </div>
            </div>
            <!-- Mini Stats -->
            <div class="col-xl-4">
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    <div class="card-body">
                        <h6 class="text-muted text-uppercase fw-bold small mb-3">Summary Produk</h6>
                        <div class="d-flex justify-content-between mb-2">
                            <span>Slipsheet</span>
                            <span class="fw-bold text-primary" id="val-slipsheet">0</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <span>Terpal</span>
                            <span class="fw-bold text-primary" id="val-terpal">0</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <span>Pallet</span>
                            <span class="fw-bold text-info" id="val-pallet">0</span>
                        </div>
                        <hr class="my-2">
                        <div class="d-flex justify-content-between mb-2">
                            <span>PPN</span>
        ...
        <div class="row mt-4">
            <div class="col-xl-6">
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-header bg-transparent border-0 pt-4 px-4">
                        <h5 class="card-title mb-0 fw-bold">Perbandingan Bulanan (Total Qty)</h5>
                    </div>
                    <div class="card-body">
...
<script>
    $(document).ready(function() {
        ...
        function loadData() {
            ...
            $.ajax({
                url: "/api/dashboard/tkbm/bps/get-stats",
                data: {
                    year,
                    month
                },
        ...
        function updateWidgets(stats) {
            $('#stat-total-qty').text(stats.total_qty.value);
            $('#stat-total-produk').text(stats.total_produk);
            $('#stat-total-fee').text('Rp ' + stats.total_fee);
            $('#stat-grand-total').text('Rp ' + stats.grand_total);

            $('#val-slipsheet').text(stats.total_slipsheet);
            $('#val-pallet').text(stats.total_pallet);
            $('#val-terpal').text(stats.total_terpal);
            $('#val-ppn').text('Rp ' + stats.ppn);
            $('#val-pph').text('Rp ' + stats.pph);

            const trendHtml = stats.total_qty.trend === 'up' ?
                `<i class="bx bx-trending-up me-1"></i> ${stats.total_qty.percent}%` :
                `<i class="bx bx-trending-down me-1"></i> ${stats.total_qty.percent}%`;

            $('#stat-qty-trend').html(trendHtml);
        }
        ...
        function renderDailyChart(data) {
            dailyChart = Highcharts.chart('daily-trend-chart', {
                ...
                yAxis: [{ // Primary yAxis
                    title: {
                        text: 'Quantity Produk'
                    },
                    gridLineColor: 'rgba(0,0,0,0.05)'
                }, { // Secondary yAxis
                    title: {
                        text: 'Jumlah Buruh (TKBM)'
                    },
                    opposite: true,
                    gridLineWidth: 0
                }],
                ...
                series: [{
                    name: 'Terpal',
                    type: 'areaspline',
                    data: data.map(i => parseFloat(i.total_terpal)),
                    color: '#6366f1',
                    tooltip: {
                        valueSuffix: ' Pcs'
                    }
                    ...
                    name: 'Slipsheet',
                    type: 'areaspline',
                    data: data.map(i => parseFloat(i.total_slipsheet)),
                    color: '#10b981',
                    tooltip: {
                        valueSuffix: ' Pcs'
                    }
                    ...
                    type: 'line',
                    yAxis: 1,
                    data: data.map(i => parseFloat(i.total_tkbm)),
                    color: '#ef4444',
                    dashStyle: 'ShortDot',
                    tooltip: {
                        valueSuffix: ' Orang'
        ...
        function renderMonthlyChart(data) {
            monthlyChart = Highcharts.chart('monthly-trend-chart', {
                ...
                series: [{
                    name: 'Total Qty',
                    data: data.map(i => parseFloat(i.total_qty))
                }]
            });
        }
        ...
        function renderRecentEntries(entries) {
            ...
            entries.forEach(item => {
                ...
                html += `
                    <tr>
                        <td class="ps-4">
                            <div class="fw-bold small">${item.date}</div>
                            <div class="text-muted" style="font-size: 11px;">${item.petugas || 'System'}</div>
                        </td>
