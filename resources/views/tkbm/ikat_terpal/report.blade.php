    ...
    <script>
        $(document).ready(function() {
            ...
            function loadData(params = {}) {
                // Hapus semua data row sebelumnya
                tableBody.find('tr:not(#loadingRow):not(#emptyRow)').remove();
                showLoading();

                $.ajax({
                    url: `{{ url('api/tkbm/get-data/ikat-terpal') }}`,
                    method: 'GET',
                    data: params,
                    dataType: 'json',
                    success: function(response) {
                        ...
                        if (response.status === 'success' && response.data && response.data.length >
                            ...
                            response.data.forEach((item, index) => {
                                ...
                                const row = `
                                    <tr class="data-row">
                                        ...
                                        <td>Rp ${fmtID(item.subtotal_barang)}</td>
                            ...
                            const ppnAct = Math.round(sumTotalFee * (PPN / 100));
                            const pphAct = Math.round(sumTotalFee * (PPH / 100));
                            const grandTotal = Math.round(sumTotalQty + sumTotalFee + ppnAct - pphAct);
                            ...
                            showEmpty();
                            // Reset totals jika tidak ada data
                            $('#fee-value').text('0%');
                            $('#qtyPallet').text('0');
                            $('#totalQty').text('0');
                            $('#totalFee').text('0');
            ...
            $('#btn-reset').on('click', function() {
                formFilter[0].reset();
                loadData(); // reload tanpa parameter (akan ambil data bulan ini)
            });
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.status === 'success') {
                                    ...
                                    // Reload dengan parameter filter yang sama jika ada
                                    const startDate = $('#start_date').val();
                                    const endDate = $('#end_date').val();
                                    const params = {};
                                    if (startDate && endDate) {
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                const url = `{{ url('/tkbm/ikat-terpal/show') }}/${id}`;
            ...
            $('#btn-update').on('click', function() {
                ...
                const url = `{{ url('/tkbm/ikat-terpal/update') }}/${id}`;
