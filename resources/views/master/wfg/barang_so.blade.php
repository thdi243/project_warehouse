    ...
    <script>
        $(document).ready(function() {
            ...
            $("#formBarang").on("submit", function(e) {
                ...
                let url = "/master/wfg/barang/store";
                let method = "POST";

                if (itemId) {
                    url = `/master/wfg/barang/update/${itemId}`;
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/master/wfg/barang/delete/${id}`,
                            method: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            $(document).on("click", ".approve-item", function() {
                ...
                $.ajax({
                    url: `/master/wfg/barang/new/approve/${id}`,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
            ...
            $(document).on("click", ".reject-item", function() {
                ...
                $.ajax({
                    url: `/master/wfg/barang/new/reject/${id}`,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
        ...
        function showDetailCollapse(id) {
            ...
            let detailHTML = `
                <div class="row g-3 text-wrap">
                    ...
                    <div class="col-md-4">
                        <strong>Dibuat Oleh:</strong> ${item.created_by?.nama_lengkap ?? '-'}
                    </div>
                </div>
            `;
        ...
        function editBarang(id) {
            $.get(`/api/wfg/show/barang/${id}`, function(res) {
                let item = res.data;
                if (item) {
                    $("#itemId").val(item.id);
        ...
        function restoreBarang(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/master/wfg/barang/restore/${id}`,
                        method: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function forceDeleteBarang(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/master/wfg/barang/force-delete/${id}`,
                        method: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
