            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0"><i class="mdi mdi-upload me-2"></i>Form Stock Opname WPM</h4>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#btnSaveFinal').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('wpm.stock_opname.process') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
        ...
        function loadItems() {
            ...
            container.html(`
                    <div class="text-center py-5 text-primary">
                        ...
                        <p class="fw-semibold">Mengambil data opname Packaging Material...</p>
                    </div>
                `);
                    ...
                    if (res.status === 'success' && res.data.length > 0) {
                        ...
                        container.html(`
                            <div class="text-center py-5 text-muted">
                                ...
                                <p>Silakan unggah data SOH Packaging Material terlebih dahulu sebelum melakukan opname.</p>
                            </div>
                        `);
                    }
                }
        ...
        function attachRowEvents() {
            ...
            $('.qty-full, .qty-receh, .notes-field').on('change', function() {
                ...
                $.ajax({
                    url: "{{ route('wpm.stock_opname.save-temp') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        soh_id: sohId,
        ...
        function resetRow(sohId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('wpm.stock_opname.reset-temp-row') }}",
                        type: "DELETE",
                        data: {
                            _token: "{{ csrf_token() }}",
                            soh_id: sohId,
        ...
        function editTemp(sohId) {
            $.ajax({
                url: "{{ route('wpm.stock_opname.getDataTempEdit', ':id') }}".replace(':id', sohId),
                type: 'GET',
                success: function(res) {
                    if (res.status === 'success') {
        ...
        $('#saveEditBtn').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('wpm.stock_opname.update-temp') }}",
                method: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    items: updates,
