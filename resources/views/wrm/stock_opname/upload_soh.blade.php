            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0"><i class="mdi mdi-upload me-2"></i>Upload Stock On Hand</h4>
                            ...
                            <div class="col-lg-6 col-md-12 mb-3 mb-lg-0">
                                <form id="searchSOHForm" class="position-relative w-100" role="search">
                                    <input type="search" class="form-control ps-5" id="searchSOHInput"
                                        placeholder="Cari MID, nama barang, atau No SPB..." aria-label="Search">
                                    <i
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title text-white" id="uploadModalLabel">
                        <i class="mdi mdi-file-excel-outline me-1"></i> Upload File SOH
                    </h5>
                ...
                <form id="uploadSOHForm" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer bg-light d-flex gap-2">
                        <a href="{{ route('wrm.stock_opname.soh.template') }}" class="btn btn-primary flex-grow-1">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#uploadSOHForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('wrm.stock_opname.soh.import') }}",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.data && res.data.length > 0) {
                        ...
                        tableBody.append(`
                            <tr>
                                <td colspan="9" class="text-center py-4 text-muted">Tidak ada data SOH hari ini.</td>
                            </tr>
                        `);
                        $('#tableInfo').text('');
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                <div class="bg-light rounded-3 p-3 border">
                                    ...
                                    <strong class="text-success fs-5">${item.qty_unrest.toLocaleString('id-ID')}</strong>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-light rounded-3 p-3 border">
                                    <div class="small text-muted mb-1 text-uppercase">QI</div>
                                    <strong class="text-info fs-5">${item.qty_qi.toLocaleString('id-ID')}</strong>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-light rounded-3 p-3 border">
                                    <div class="small text-muted mb-1 text-uppercase">BLOCKED</div>
                                    <strong class="text-danger fs-5">${item.qty_block.toLocaleString('id-ID')}</strong>
                                </div>
                            </div>
                        </div>
                    </div>                  
        ...
        function deleteSOH(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ route('wrm.stock_opname.soh.delete', '') }}/${id}`,
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
