            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0"><i class="mdi mdi-upload me-2"></i>Form Stock Opname WRM</h4>
                ...
                <div class="modal-body p-4">
                    <form id="formAddItem">
                        ...
                        <div class="p-3 rounded-3 mb-3 bg-light">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select select2" id="no_spb_item" name="no_spb" required
                                        style="width:100%">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" class="form-control" id="pallet_item" name="pallet"
                                        placeholder="Ketik Pallet..." required>
                                </div>
                            </div>
                        </div>
                        <div class="p-3 rounded-3 mb-3 bg-light-subtle border">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <label class="form-label">UNREST (Kg)</label>
                                    <input type="number" class="form-control" name="unrest" value="0"
                                        min="0" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">QI (Kg)</label>
                                    <input type="number" class="form-control" name="qi" value="0"
                                        min="0" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">BLOCKED (Kg)</label>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6 d-flex align-items-center">
                                    <div class="form-check form-switch mt-4">
                                        <input class="form-check-input" type="checkbox" id="add_qty_full"
                                            name="qty_full" value="1" checked style="width: 40px; height: 20px;">
                                        <label class="form-check-label fw-semibold ms-2" for="add_qty_full">Terhitung /
                                            Checked</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Modal change barang event
            $('#mid_barang').on('change', function() {
                const mid = $(this).val();
                const spbSelect = $('#no_spb_item');

                spbSelect.html('<option value="">-- Pilih atau Ketik No SPB --</option>');
            ...
            $('#btnSaveNewItem').on('click', function() {
                const isChecked = $('#formAddItem [name="qty_full"]').is(':checked');
                ...
                let formDataArray = $('#formAddItem').serializeArray();
                let hasQtyFull = false;
                formDataArray.forEach(function(item) {
                    if (item.name === 'qty_full') {
                        hasQtyFull = true;
                    }
                });
                if (!hasQtyFull) {
                ...
                $.ajax({
                    url: "{{ route('wrm.stock_opname.save-temp-new') }}",
                    type: "POST",
            ...
            $('#btnSaveFinal').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('wrm.stock_opname.process') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        ...
                        if (res.status === 'success') {
                            ...
                            let diffList = res.analysis.filter(item => item.selisih !== 0).map(
                                ...
                                `<tr>
                                    ...
                                    <td class="text-center font-monospace">${item.no_spb ? item.no_spb : '-'}</td>
                            ...
                            let list = res.issues.map(item =>
                                `<li>MID: ${item.mid} | SPB: ${item.no_spb ? item.no_spb : '-'} | Selisih: ${item.selisih} Kg</li>`
                            ).join('');
                            Swal.fire({
        ...
        function loadItems() {
            ...
            container.html(`
                    <div class="text-center py-5 text-primary">
                        ...
                        <p class="fw-semibold">Mengambil data opname Raw Material...</p>
                    </div>
                `);

            $.ajax({
                url: "{{ route('wrm.stock_opname.getData') }}",
                type: "GET",
                data: {
                    jenis_so: jenisSo
                },
                success: function(res) {
                    if (res.status === 'success' && res.data.length > 0) {
                        ...
                        res.data.forEach((item, index) => {
                            const isCounted = item.qty_full !== null;
                            const qtyFull = item.qty_full;
                            const qtyReceh = item.qty_receh;
                            ...
                            html += `
                                <tr class="soh-row align-middle" data-id="${item.soh_id}" data-qty-kg="${item.qty_kg}" data-qty-soh="${item.qty_soh}" data-summary-db="${summaryVal}">
                                    ...
                                    <td class="text-center">
                                        <input type="number" class="form-control text-center qty-receh" value="${qtyReceh !== null ? qtyReceh : ''}" min="0" placeholder="0">
                                    </td>
                        ...
                        container.html(`
                            <div class="text-center py-5 text-muted">
                                ...
                                <p>Silakan unggah data SOH Raw Material terlebih dahulu sebelum melakukan opname.</p>
                            </div>
                        `);
                    }
                }
            ...
            function saveRow(row) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        soh_id: sohId,
                        qty_full: qtyFullVal,
                        qty_receh: qtyRecehValStr !== '' ? qtyReceh : null,
                        keterangan: noteVal,
        ...
        function resetRow(sohId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('wrm.stock_opname.reset-temp-row') }}",
                        type: "DELETE",
                        data: {
                            _token: "{{ csrf_token() }}",
                            soh_id: sohId,
        ...
        function loadAllTempData() {
            ...
            $.ajax({
                url: "{{ route('wrm.stock_opname.getTempBatch') }}",
                type: "GET",
                data: {
                    soh_ids: sohIds,
                    jenis_so: $('#jenis_so').val()
                ...
                success: function(res) {
                    if (res.status === "success" && Array.isArray(res.data)) {
                        ...
                        rows.each(function() {
                            $(this).find('.physical-summary').text('-');
                            $(this).attr('data-summary-db', 0);
                            $(this).find('.diff-indicator-dot').removeClass(
                        ...
                        for (const key in groupedData) {
                            ...
                            if (row.length) {
                                ...
                                const latestHistory = tempItem.history[0];
                                if (latestHistory) {
                                    row.find('.qty-full').prop('checked', latestHistory.qty_full === 1);
                                    row.find('.qty-receh').val(latestHistory.qty_receh !== null ? latestHistory
                                        .qty_receh : '');
                                } else {
        ...
        function editTemp(sohId) {
            $.ajax({
                url: "{{ route('wrm.stock_opname.getDataTempEdit', ':id') }}".replace(':id', sohId),
                type: 'GET',
                success: function(res) {
                    if (res.status === 'success') {
                                ...
                                <div class="mb-3 border border-info p-3 rounded temp-item bg-light" data-id="${item.id}" data-qty-kg="${qtyKg}">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select qty_full">
                                                <option value="1" ${item.qty_full === 1 ? 'selected' : ''}>Terhitung</option>
                                                <option value="0" ${item.qty_full === 0 ? 'selected' : ''}>Tidak Terhitung</option>
                                            </select>
                                        </div>
                                        <div class="col-md-6">
                        ...
                        <div class="col-md-3">
                            <div class="p-2 border border-info rounded h-100 bg-light fade show history-card">
                                <div class="fw-semibold text-dark mb-1 d-flex justify-content-between align-items-center">
                                    <span>Checklist: ${h.qty_full === 1 ? 'Terhitung' : 'Tidak'}, Qty: ${h.qty_receh} Kg</span>
        ...
        $(document).on('click', '#editModal .btn-delete-temp', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('wrm.stock_opname.delete-temp', ':id') }}".replace(':id',
                            id),
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}",
        ...
        $('#saveEditBtn').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('wrm.stock_opname.update-temp') }}",
                method: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    items: updates,
