...
@section('content')
    ...
    <div class="modal fade" id="editP2HModal" tabindex="-1">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <form id="editP2HForm" enctype="multipart/form-data">
                    @csrf
        ...
        $(document).ready(function() {
                ...
                Object.entries(item.shifts).forEach(([shift, detail]) => {
                    ...
                    for (const [key, value] of Object.entries(detail)) {
                        if (['id', 'created_at', 'updated_at', 'jenis_p2h', 'operator_name',
                                'p2h_model_id', 'shift', 'foto_kondisi_accu'
                    ...
                    // Render foto_kondisi_accu at the very end of the list
                    const photoValue = detail.foto_kondisi_accu;
                    let photoBadge = '';
                    if (photoValue) {
                        photoBadge = `
                            <a href="${storageUrl}/${photoValue}" target="_blank" class="d-block mt-1">
                                <img src="${storageUrl}/${photoValue}" class="img-thumbnail" style="max-height: 80px;" alt="Foto Accu"/>
                            </a>
                        `;
                    } else {
                        photoBadge = `<span class="text-muted">-</span>`;
                    }

                    html += `
                        <div class="col-md-4 mb-2">
                            <strong>Foto Kondisi Accu</strong><br>${photoBadge}
                        </div>
                    `;
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                [1, 2, 3].forEach(shift => {
                    ...
                    if (shiftData.foto_kondisi_accu) {
                        currentPhotoHtml = `
                        <div class="mt-2">
                            <label class="form-label d-block"><small>Foto Accu Saat Ini:</small></label>
                            <a href="${storageUrl}/${shiftData.foto_kondisi_accu}" target="_blank">
                                <img src="${storageUrl}/${shiftData.foto_kondisi_accu}" class="img-thumbnail" style="max-height: 100px;" />
                            </a>
                        </div>
                    `;
                    }
