...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            function loadIncomingData(filterDate = null) {
                $.ajax({
                ...
                pageData.forEach((inc, index) => {
                    tbody.append(`
                        <tr>
                            ...
                            <td>${inc.po_number}</td>
                            <td>${inc.po_date}</td>
                            <td><span class="badge badge-soft-success py-2 px-3">${inc.gr_qty ?? '-'}</span></td>
                            <td>${inc.gr_date}</td>
