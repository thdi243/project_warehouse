            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0"><i class="mdi mdi-upload me-2"></i>Form Stock Opname WSP</h4>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btnStartOpname').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('wsp.stock_opname.startOpname') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
            ...
            $('#btnAddRow').on('click', function() {
                $('#formAddItem')[0].reset();
                $('#mid_barang').val(null).trigger('change');
            ...
            $('#btnSaveNewItem').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('wsp.stock_opname.save-temp-new') }}",
                    type: "POST",
            ...
            $('#btnSaveFinal').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('wsp.stock_opname.process') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
        ...
        function checkSessionStatus() {
            $.get("{{ route('wsp.stock_opname.getStatusOpname') }}", function(res) {
        ...
        function loadItems() {
            ...
            $.ajax({
                url: "{{ route('wsp.stock_opname.getData') }}",
                type: "GET",
                success: function(res) {
        ...
        function resetRow(sohId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('wsp.stock_opname.reset-temp-row') }}",
                        type: "DELETE",
                        data: {
                            _token: "{{ csrf_token() }}",
                            soh_id: sohId
        ...
        function loadAllTempData() {
            ...
            $.ajax({
                url: "{{ route('wsp.stock_opname.getTempBatch') }}",
                type: "GET",
                data: {
                    soh_ids: sohIds
                },
        ...
        function editTemp(sohId) {
            $.ajax({
                url: "{{ route('wsp.stock_opname.getDataTempEdit', ':id') }}".replace(':id', sohId),
                type: 'GET',
                success: function(res) {
                    if (res.status === 'success') {
        ...
        $(document).on('click', '#editModal .btn-delete-temp', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('wsp.stock_opname.delete-temp', ':id') }}".replace(':id',
                            id),
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}",
        ...
        $('#saveEditBtn').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('wsp.stock_opname.update-temp') }}",
                method: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    items: updates,
