            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0"><i class="mdi mdi-file-document-outline me-2"></i>Report Stock Opname WSP
                        </h4>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btnSubmitApproval').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('wsp.stock_opname.send-approval') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        so_id: soId,
        ...
        function loadPendingApprovals() {
            $.ajax({
                url: "{{ route('wsp.stock_opname.report.pending-approval') }}",
                type: "GET",
                dataType: "json",
                success: function(res) {
                    if (res.status === 'success') {
        ...
        window.viewDetailReport = function(id) {
            $.ajax({
                url: "{{ route('wsp.stock_opname.report.detail', ':id') }}".replace(':id', id),
                type: 'GET',
                success: function(res) {
                    if (res.status === 'success') {
        ...
        window.editReportRow = function(id) {
            $.ajax({
                url: "{{ route('wsp.stock_opname.report.detail', ':id') }}"
                    .replace(':id', id),
                type: 'GET',
                success: function(res) {
                    if (res.status === 'success') {
        ...
        $('#formEditReport').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('wsp.stock_opname.report.update', ':id') }}"
                    .replace(':id', id),
                type: 'POST',
                data: $(this).serialize(),
                success: function(res) {
        ...
        window.deleteReportDetail = function(detailId, buttonEl) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('wsp.stock_opname.report.detail.delete', ':id') }}"
                            .replace(':id', detailId),
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
        ...
        window.deleteReportRow = function(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('wsp.stock_opname.report.delete', ':id') }}"
                            .replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
        ...
        function checkApprovalStatus(sop, date) {
            ...
            $.get("{{ route('wsp.stock_opname.approval.show', '') }}/" + sop.id, function(
                res) {
        ...
        function actionApproval(actionStatus) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.showLoading();
                    $.ajax({
                        url: "{{ route('wsp.stock_opname.update.status-approval') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            so_id: soId,
        ...
        function loadApproverOptions(callback) {
            ...
            $.ajax({
                url: "{{ route('wsp.stock_opname.getDataApproval') }}",
                type: 'GET',
                success: function(res) {
                    foremanSelect.html(
                        '<option value="">-- Pilih Foreman --</option>');
